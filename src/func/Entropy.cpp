...
void computeKmerTable2(vector<Sequence>& gene, long K, KmerTable2 &kmerTable) {
    ...
    // start_time = std::chrono::high_resolution_clock::now();
    // int i = 0;
    // for (auto &kmer : kmer_based_table) {
    //     long sum = 0;
    //     for (size_t i = 0; i < kmer.second.size(); i++) {
    //         sum += kmer.second[i];
    //     }
    //     float entropy = 0;
    //     for (size_t i = 0; i < kmer.second.size(); i++) {
    //         float p = (float)kmer.second[i] / sum;
    //         if (p > 0) {
    //             entropy += p * log2(p);
    //         }
    //     }
    //     entropy = -entropy;
    //     kmerTable.entropy.insert(make_pair(kmer.first, entropy));
        
    //     print_progress_bar("ComputeEntropy", i, kmerTable.n_kmer_total, start_time);
    //     i += 1;
    // }
    // kmer_based_table.clear();
    // int i = 0;
    // for (auto &kmer: kmer_occurance) {
    //     long sum = 0;
    //     for (size_t i = 0; i < kmerTable.n_gene; i++) {
    //         if (kmerTable.count[i].find(kmer) != kmerTable.count[i].end()) {
    //             sum += kmerTable.count[i][kmer];
    //         }
    //     }
    //     float entropy = 0;
    //     for (size_t i = 0; i < kmerTable.n_gene; i++) {
    ...
    //             continue;
    //         }
    //         float p = (float)kmerTable.count[i][kmer] / sum;
    //         if (p > 0) {
    //             entropy += p * log2(p);
    //         }
    //     }
