...
PQuantParams::PQuantParams(cxxopts::ParseResult &result) {
    ...
    if (result["gene"].as<std::string>().size() == 0 && result["read"].as<std::string>().size() == 0 && result["data"].as<std::string>().size() > 0) {
        ...
        if (datasetMap.find(this->data) == datasetMap.end()) {
            std::vector<std::string> TARGET_RQUIRES_DATA = {"STEP1", "STEP2", "STEP3", "STEP4", "STEP5", "all", "table", "fasta"};
            std::vector<std::string>::iterator it = find(TARGET_RQUIRES_DATA.begin(), TARGET_RQUIRES_DATA.end(), this->target);
            if (it != TARGET_RQUIRES_DATA.end()) {
    ...
    if (filename_kmerTable.size() == 0 && filename_kmerList.size() == 0 && path_kmer_folder.size() > 0) {
        filename_kmerTable = path_kmer_folder + "/kmertable_" + data + "_" + std::to_string(k) + "_" + std::to_string(thres);
        filename_kmerList =  path_kmer_folder + "/kmerlist_" + data + "_" + std::to_string(k) + "_" + std::to_string(thres);
        if (this->json_format) {
            filename_kmerTable += ".txt";
            filename_kmerList += ".txt";
        } else {
            filename_kmerTable += ".bin";
            filename_kmerList += ".bin";
        }
    }
