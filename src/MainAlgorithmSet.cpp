...
void MainAlgorithmSet::encodeAndEncryptSimulatedReads(PQuantParams &param) {
    // print warning that this is for simulated reads
    std::cout << " === Warning ===" << endl;
    std::cout << " === Warning ===" << endl;
    std::cout << "This function is for simulated reads" << endl;
    std::cout << " === Warning ===" << endl;
    std::cout << " === Warning ===" << endl;
    ...
    vector<Sequence> seq_vec;
    cout << "=== read fasta file (reference) ===" << endl;
    readFastaFile(param.filename_ref, seq_vec);
    vector<Sequence> reads_seq;
    ...
    reads_seq.clear();
    auto end_time_read = std::chrono::high_resolution_clock::now();
    auto duration_kmerTableRead = std::chrono::duration_cast<std::chrono::milliseconds>(end_time_read - start_time_read).count();
    std::cout << "computeKmerTableRead duration = " << duration_kmerTableRead << " ms" << std::endl;
    if (param.verbose) {
        kmerTableRead.print();
    }
    std::cout << std::endl;
    ...
    auto total_duration = duration_simulateRead + duration_kmerTableRead + duration_kmerList + duration_encode;
    std::cout << " === Duration summaries ===" << endl;
    std::cout << "simulating read duration = " << duration_simulateRead << " ms" << endl;
    std::cout << "kmerTableRead duration = " << duration_kmerTableRead << " ms" << endl;
    std::cout << "load kmerList duration = " << duration_kmerList << " ms" << endl;
    std::cout << "encodeRead duration = " << duration_encode << " ms" << endl;
    if (param.operate_then_serialize) {
        std::cout << "EncryptReadKmer duration = " << duration_encrypt << " ms" << endl;
