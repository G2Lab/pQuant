...
cd /gpfs/commons/groups/gursoy_lab/shong/Github/pQuant_enc
...
# Step 1
JOBID_STEP1=$(sbatch --output="${SLURM_OUT_FOLDER}/step1.txt" --parsable run_single_step.sh STEP1 "${GLOBAL_ARGUMENT}" )
echo "STEP1: submitted job ${JOBID_STEP1}"

# Step 2 (waits for Step 1 to finish)
JOBID_STEP2=$(sbatch --output="${SLURM_OUT_FOLDER}/step2.txt" --dependency=afterany:${JOBID_STEP1} --parsable run_single_step.sh STEP2 "${GLOBAL_ARGUMENT}")
echo "STEP2: submitted job ${JOBID_STEP2}"

# Step 3 (waits for Step 2 to finish)
JOBID_STEP3=$(sbatch --output="${SLURM_OUT_FOLDER}/step3.txt" --dependency=afterany:${JOBID_STEP2} --parsable run_single_step.sh STEP3 "${GLOBAL_ARGUMENT}")
...
do
    ...
    JOBID_STEP4=$(sbatch --output="${STEP4_FOLDER}/step4_${i}.txt" --dependency=afterany:${JOBID_STEP3} --array=${i} --parsable run_single_step.sh STEP4 "${GLOBAL_ARGUMENT} --gs ${GENE_START} --ge ${GENE_END}")
...
JOBID_STEP5=$(sbatch --output="${SLURM_OUT_FOLDER}/step5.txt" --dependency=afterany:${DEPENDENCY_STEP4} --parsable run_single_step.sh STEP5 "${GLOBAL_ARGUMENT} --gs 0 --ge ${N_GENES}")
